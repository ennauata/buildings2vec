...
class JunctionTest(data.Dataset):
    def __init__(self, H, split='test', size_info=False):
        self.split = split
        self.filelst = ref.data_root / 'all_list.txt'
        self.img_dir = ref.data_root / 'rgb'
        ...
        with open(self.filelst, 'r') as f:
            ...
            print(len(filenames))
            self.imgnames = [t+'.jpg' for t in filenames if Path.is_file(self.img_dir.joinpath(t+'.jpg'))]
...
class Junction(data.Dataset):
    ...
    def __getitem__(self, index):
        imgname = self.imgnames[index]
        with open(self.src_dir.joinpath(imgname + self.ext), 'rb') as handle:
