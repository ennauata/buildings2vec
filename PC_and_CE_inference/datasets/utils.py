...
def make_crop(d, bbox, suffix):
    ...
    d_crop['imgname'] = "{}_{}".format(d['imgname'], suffix[0])
...
def make_mirror(d, axis=1):
    ...
    d_mirror['imgname'] = "{}_{}".format(d['imgname'], suffix)
...
def save_ann(H, d, save_dir, max_len = 1, split='train'):
    ...
    for fn in outputs:
        ...
        with open('{}/{}.pickle'.format(dir_to_save, imgname), 'wb') as handle:
            pickle.dump(fn, handle, protocol=pickle.HIGHEST_PROTOCOL)
        cv2.imwrite(join(dir_to_save, fn['imgname']+'.jpg'), fn['image'])
...
def load_data_and_save(H, src_dir, save_dir, filenames, split='train', use_mp=True):
    ...
    imgnames = [x[:-4] for x in os.listdir(dirname) if valid_pickle_file(x)]
    train, val, test = [], [], []
    ...
    trainImgs = read_list_from_file(ref.data_root / 'train_list.txt')
    testImgs = read_list_from_file(ref.data_root / 'valid_list.txt')
...
def create_dataset(H, split, use_mp = True):
    ...
    with open(ref.data_root / '{}_list.txt'.format(split), 'r') as f:
        filelst = f.read().splitlines()
    
    filelst = [x + '.pkl' for x in filelst]
