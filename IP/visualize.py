...
res_dir = '/home/nelson/Workspace/building_reconstruction/working_model/wireframe/result/junc/3/15/1'
...
for _id in _ids:
    ...
    # draw annotations
    dwg = svgwrite.Drawing('../result/svg/{}_0.svg'.format(_id), (128, 128))
    ...
    # draw all detected junctions
    dwg = svgwrite.Drawing('../result/svg/{}_1.svg'.format(_id), (128, 128))
    dwg.add(svgwrite.image.Image(im_path, size=(128, 128)))
    im_path = os.path.join(rgb_dir, _id + '.jpg')
    draw_junctions(dwg, cs, th, th_c)
    dwg.save()
    ...
    dwg = svgwrite.Drawing('../result/svg/{}_2.svg'.format(_id), (128, 128))
 ...
 #    dwg = svgwrite.Drawing('../result/svg/{}_3.svg'.format(_id), (128, 128))
 #    dwg.add(svgwrite.image.Image(edge_map_path, size=(128, 128)))
 #    im_path = os.path.join(rgb_dir, _id + '.jpg')
 #    draw_building(dwg, junctions, juncs_on, lines_on)
 #    dwg.save()
 ...
 #    metrics[1].forward(graph_annot, junctions, juncs_on, lines_on, _id)
   ...
   #  deb.save('./regions/{}.jpg'.format(_id))
   #  dwg = svgwrite.Drawing('../result/svg/{}_4.svg'.format(_id), (128, 128))
   #  dwg.add(svgwrite.image.Image(os.path.abspath('./regions/{}.jpg'.format(_id)), size=(128, 128)))
   #  im_path = os.path.join(rgb_dir, _id + '.jpg')
   #  draw_building(dwg, junctions, juncs_on, lines_on)
   #  dwg.save()
...
for k, m in enumerate(metrics):
	print('experiment %d'%(k+1))
	m.print_metrics()

#metrics[4].print_metrics()
#################### BACKUP ####################
# for fname in files:
#   #if '1554160388.09' in fname:
#   _id = fname.split('/')[-1].replace('_False.npy', '')
#   with open(fname, 'rb') as f:
#       c = p.load(f, encoding='latin1')

#   edge_map = np.array(Image.open('{}/{}.jpg'.format(edge_dir, _id)).convert('L'))/255.0
#   junctions, juncs_on, lines_on = reconstructBuilding(c['junctions'], c['thetas'], edge_map)

#   im_path = '{}/{}.jpg'.format(rgb_dir, _id)
#   dwg = svgwrite.Drawing('../result/svg/{}.svg'.format(_id), (128, 128))
#   dwg.add(svgwrite.image.Image(im_path, size=(128, 128)))
#   im_path = os.path.join(rgb_dir, _id + '.jpg')
#   draw_junctions(dwg, c['junctions'], c['thetas'])
#   dwg.save()
...
# fname = '{}/{}.npy'.format(corner_dir, _id)
# cs = np.load(fname)[:, ::-3]
# shift = 5.0
# xs = cs[:, 0]-shift
# ys = cs[:, 1]+shift
