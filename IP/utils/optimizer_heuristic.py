...
def reconstructBuildingBaseline(junctions, edge_map, regions=None, with_weighted_junctions=True, with_corner_variables=False, with_edge_confidence=False, 
    ...
    if with_edge_confidence:
        ...
        for k, l in ls_list:
            ...
            obj += 10*(np.prod([corner_confs[k], corner_confs[l], lw])-corner_edge_thresh)*ls_var_dict[(k, l)] # favor edges with over .5?
    ...
    if use_regions:
        ...
        for i, reg in enumerate(regions):
            ...
            if np.array(inds).shape[0] > 0:
                ...
                inds = np.linspace(0, reg_contour[i].shape[0], min(50, reg_contour[i].shape[0]), endpoint=False).astype('int')
        ...
        for i in reg_list:
            ...
            for pt, th in zip(pts, ths):
                ...
                if not intersec_region:
                    ...
                    m.addConstr(sum_in_set*reg_var_ls[i] <= 1 + slack_var, "r2_{}_{}".format(th, i))
                    obj -= slack_var
                ...
                intersec_set, region_id, self_intersect = castRayRegion(pt, th, ls_list, junctions, reg_sm[i], regions[i], other_regions, other_regions_id)
                if (region_id is not None) and (not self_intersect):
                    ...
                    slack_var = m.addVar(vtype=GRB.INTEGER, name="slack_var_inter_{}_{}".format(th, i))
        ...
        for j1 in js_list:
            ...
            for i in range(len(thetas[j1])):
                ...
                if use_junctions_with_var:
                    ...
                    obj += junc_th_var * (np.min([corner_confs[j1], theta_confs[j1][i]]) - theta_threshold)
...
def castRayRegion(pt, th, ls_list, junctions, reg_i, large_reg, other_regs, other_regs_id, ray_length=1000.0, thresh=0.0):
    ...
    for _id, reg_j in zip(other_regs_id, other_regs):
        reg_j_tag = np.array(reg_j)
        inds = np.where(reg_j_tag>0)
        reg_j_tag[inds] = _id+1
        comb_reg += reg_j_tag
        background += reg_j
    background += large_reg
    ...
    if inds.shape[1] > 0:
        ...
        # # DEBUG
        # dr.ellipse((xc-2, yc-2, xc+2, yc+2), fill='red')
        # print(region_id)
        # # DEBUG
        ...
        dr.line((x1, y1, xc, yc), fill='white', width=8)
        dr.ellipse((x1-4, y1-4, x1+4, y1+4), fill='black')
        ray = np.array(ray_im)/255.0
        self_intersect = np.logical_and(ray, reg_i).sum()/(ray.sum()+1e-8)
        self_intersect = self_intersect > thresh

        # render ray
        ray_im = Image.new('L', (256, 256))
        draw = ImageDraw.Draw(ray_im)
        draw.line((x1, y1, xc, yc), fill='white', width=8)
        ray = np.array(ray_im)/255.0
