...
shared_edges_fname = '/home/nelson/Workspace/cities_dataset/shared_edges.pkl'

metrics = [Metrics(), Metrics()]
with open(shared_edges_fname, 'rb') as f:
    shared_edges = p.load(f, encoding='latin1')

# for _id, i, j in shared_edges:
#     for msk in shared_edges[(_id, i, j)]:
#         print(msk.shape)

with open('/home/nelson/Workspace/cities_dataset/complex_buildings.txt') as f:
    _ids = [x.strip() for x in f.readlines()]

for _id in _ids:

    # if _id not in '1554168442.85':
    #     continue
   ...
   ################### EXPERIMENT V ####################
    junctions, juncs_on, lines_on, regs_sm_on = reconstructBuildingBaseline(cs, edge_map,
        ...
        edge_map_weight=5.0,
        inter_region_weight=5.0,
        intersection_constraint=True,
        post_process=True,
        shared_edges=None,
        _id=_id,
        _exp_tag='without_new_constraint'
        )
   ...
   ################### EXPERIMENT VI ####################
    ...
    junctions, juncs_on, lines_on, regs_sm_on = reconstructBuildingBaseline(cs, edge_map,
        ...
        intersection_constraint=True,
        post_process=True,
        shared_edges=shared_edges_per_id,
        _id=_id,
        _exp_tag='without_new_constraint'
        )
