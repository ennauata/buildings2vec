...
def do_buildings_evaluation(dataset, predictions, box_only, output_folder, iou_types, expected_results, expected_results_sigma_tol):
    ...
    image_list = []
    n_cols = 0
    im_shared_edges = defaultdict(list)
    for idx, prediction in enumerate(predictions):
        
        # retrieve sample info
        _building_id, reg_i_idx, reg_j_idx = dataset.sample_list[idx]

        # load regions
        region_path = '{}/{}.npy'.format(region_folder, _building_id)
        regions_det = np.load(region_path)
        reg_i = regions_det[reg_i_idx][np.newaxis, :, :]
        reg_j = regions_det[reg_j_idx][np.newaxis, :, :]
        ...
        print(_building_id)
        rgb_im = Image.open('{}/{}.jpg'.format(dataset.img_dir, _building_id))
        rgb_im = np.array(rgb_im)
        inds = np.array(np.where(reg_i[0, :, :]==1))
        rgb_im[inds[0, :], inds[1, :], :] = [0, 0, 255]
        inds = np.array(np.where(reg_j[0, :, :]==1))
        rgb_im[inds[0, :], inds[1, :], :] = [255, 0, 0]
        ...
        for s, m in zip(pred_score, masks):
            
            ## save shared edges
            if m.sum() > 0 and s > 0.2:
                im_shared_edges[(_building_id, reg_i_idx, reg_j_idx)].append(m) 

                ### DEBUG ###
                inds = np.array(np.where(m>0))
                r, g, b = 0, 255, 0 #random.randint(0, 128), random.randint(0, 128), random.randint(0, 128)
                deb_im[inds[0, :], inds[1, :], :] = [r, g, b]
                ### DEBUG ###
